...
def post_sms_json(phone_number, text):
    body = {"smsText": {
    ...
    request         = oauth.Request.from_consumer_and_token(oauth_consumer, oauth_token, "POST", oauthUrls['smsSend'])
    request.sign_request(sig_hmac, oauth_consumer, oauth_token)
